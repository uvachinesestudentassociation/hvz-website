...
import { PIXEL_SECTION_BORDER, PIXEL_SECTION_PRIMARY, PIXEL_TEXT, PIXEL_TEXT_SUBTLE } from "@/components/hvz/pixel-styles"
...
export function SafeZonesContent() {
  return (
    <section id="safe-zones" className={`scroll-mt-24 border-b-8 ${PIXEL_SECTION_BORDER} ${PIXEL_SECTION_PRIMARY}`}>
                  ...
                  <ul className="space-y-2 pl-4">
                    {zone.listItems.map((item) => (
                      <li key={item} className="flex gap-2 break-words">
                        <span className={THEME.accent.link}>{"•"}</span>
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
