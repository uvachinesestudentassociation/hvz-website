...
export function SafeZonesContent() {
  return (
    <section id="safe-zones" className="scroll-mt-24 border-b-8 border-neutral-900 bg-emerald-900/10">
          ...
          <div className="space-y-4">
            {SAFE_ZONE_SECTIONS.map((zone) => (
              <PixelDisclosure key={zone.id} id={zone.id} title={zone.title}>
                ...
                {zone.footnote && (
                  <p className="text-xs text-neutral-600 break-words">{zone.footnote}</p>
                )}
              </PixelDisclosure>
            ))}
          </div>
